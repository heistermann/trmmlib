...
file_BAL = r"P:\progress\philippines\Output07BAL151017164516.RAW4G2M_sweep1.nc"
tscanSUB = 720
tscanTAG = 720
tscanBAL = 900
...
data_SUB,a = grid_radar_accum("P:/progress/philippines/composites/SUB_6h.hdf5", attrs_SUB)
data_TAG,a = grid_radar_accum("P:/progress/philippines/composites/TAG_6h.hdf5", attrs_TAG)
data_BAL,a = grid_radar_accum("P:/progress/philippines/composites/BAL_6h.hdf5", attrs_TAG)


# adjust tagaytay values
...
# compose_weighted
radarinfo = np.array([SUB_gridded, TAG_gridded, BAL_gridded])
qualityinfo = np.array([SUB_quality_gridded, TAG_quality_gridded, BAL_quality_gridded])
qualityinfo /= np.nansum(qualityinfo, axis=0)

#qualityinfo[mask] = np.nan
composite = np.nansum(radarinfo*qualityinfo, axis=0)
tmp = np.vstack([SUB_quality_gridded, TAG_quality_gridded, BAL_quality_gridded])
...
pl.subplot(2,3,6)
pl.pcolormesh(x, y, np.ma.masked_invalid(BAL_quality_gridded.reshape((len(x),len(y)))), cmap="spectral", vmin=0, vmax=1.)
pl.colorbar()
pl.xlim(xmin_, xmax_)
pl.ylim(ymin_, ymax_)
pl.grid()
...
pm = pl.pcolormesh(x, y, np.ma.masked_less(comp,0), cmap="spectral", vmax=200)
...
#comp_attrs['y'] = y
#
#wrl.io.to_hdf5("U:/radar_composites/data/Luzon_6h_SUBTAG.hdf5", comp, metadata=comp_attrs)
